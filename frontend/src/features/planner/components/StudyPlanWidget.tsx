...
export function StudyPlanWidget() {
  ...
  return (
    <div className="clean-card space-y-4">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-base font-bold text-white font-display">Adaptive Learning Engine</h2>
              {activePlanToDisplay && (
                <span
        ...
        <div className="flex items-center space-x-2 shrink-0">
          {activePlanToDisplay?.id && (
            <button
              ...
              className="px-3 py-1.5 text-xs font-semibold text-slate-200 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg transition-colors inline-flex items-center space-x-1.5"
            >
      ...
      {/* Main Content Area */}
      {isLoadingPlan ? (
        <div className="text-xs text-slate-400 text-center py-8 font-medium">Loading study plan...</div>
      ) : !activePlanToDisplay ? (
        <div className="text-center py-10 border border-dashed border-slate-800 rounded-xl space-y-3 bg-slate-900/40">
          <Sparkles className="w-8 h-8 text-indigo-400 mx-auto" />
          <div className="space-y-1">
            <p className="text-sm font-bold text-white">No Active Study Plan</p>
          ...
          <button
            onClick={handleGenerate}
            disabled={isGenerating}
            className="inline-flex items-center space-x-1.5 text-xs bg-indigo-600 hover:bg-indigo-500 text-white font-semibold px-4 py-2 rounded-lg transition-colors shadow-sm"
          >
            <Sparkles className="w-3.5 h-3.5" />
        ...
        <div className="space-y-5">
          {/* Plan Meta Bar */}
          <div className="flex flex-wrap items-center justify-between gap-2 p-3 bg-slate-900/60 rounded-lg border border-slate-800 text-xs">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-1.5 text-slate-300">
                ...
                <span>
                  <strong className="text-white">{activePlanToDisplay.totalPlannedMinutes}</strong> / {activePlanToDisplay.totalAvailableMinutes} min planned
                </span>
              </div>
            </div>
